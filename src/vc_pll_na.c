...
vc_pll_na_t *vc_pll_na_new(hmi_t *hmi, radio_t *radio) {
   ...
   view_controller_init(VIEW_CONTROLLER(vc_pll_na), hmi, radio);
   VIEW_CONTROLLER(vc_pll_na)->show = vc_pll_na_show;
...
void vc_pll_na_show(view_controller_t *vc) {
   assert (vc != NULL);
   unsigned char b1 = eeprom_read_byte(EEPROM(M110(vc->radio)), 0x22);
   unsigned char b2 = eeprom_read_byte(EEPROM(M110(vc->radio)), 0x23);
   unsigned char b3 = eeprom_read_byte(EEPROM(M110(vc->radio)), 0x24);
